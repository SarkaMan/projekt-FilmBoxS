...
formElm.addEventListener('submit', (evt) => {
  ...
  if (textElm.value.trim() === '') {
    ...
    if (checkElm.checked === false) {
      checkElm.classList.add('is-invalid');
    } else {
      formElm.innerHTML = `<p class="card-text">${textElm.value}</p>`;
    }
  }
});
